...
public class EfficientDocument extends Document {
	...
	private void processText()
	{
		...
		for(String string : tokens){			
			// if its a word then add to word count 
			if(isWord(string)) {
				wordCount++; 
				syllableCount += countSyllables(string);
				
				// If its the last word then its a sentence even if its a stand-alone word (according to out definition)
				if(string.compareTo(tokens.get(tokens.size() -1)) == 0) {
					sentenceCount++;
				}
			}
			
			// else if its a . or ! or ?? that means its a sentence 
			else if(!isWord(string)) {
				sentenceCount++;
			}
			
			this.numSentences = sentenceCount;
			this.numWords = wordCount;
			this.numSyllables = syllableCount;
		}
	}
	...
	public static void main(String[] args)
	{
		...
		testCase(new EfficientDocument("Lorem ipsum dolor sit amet, qui ex choro quodsi moderatius, nam dolores explicari forensibus ad."),
		         32, 15, 1);
		testCase(new EfficientDocument("Me nyam a di cake. It was bless up!"), 9, 9, 2);
